...
type Reward struct {
	...
	ReasonLocationId int64     `gorm:"type:varchar(45);not null"`
	CreatedAt        time.Time `gorm:"type:datetime;not null"`
	UpdatedAt        time.Time `gorm:"type:datetime;not null"`
}
...
func (ur *UserRecommendRepo) CreateUserRecommend(ctx context.Context, u *biz.User, recommendUser *biz.UserRecommend) (*biz.UserRecommend, error) {
	...
	if 0 < recommendUser.UserId {
		...
		if 1 < len(recommendUser.RecommendCode) {
...
// LocationReward 在事务中使用，退款
func (ub *UserBalanceRepo) LocationReward(ctx context.Context, userId int64, amount int64, locationId int64, myLocationId int64) (int64, error) {
	var err error
